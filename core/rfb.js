...
/* [module] export default */ function RFB(defaults) {
    ...
    // OpenVNC (which is used by Dell BMCs) won't send us FBU if we ask for COPYRECT, TIGHT, or TIGHT_PNG first during
    // SetEncodings message to server.
...
(function() {
    ...
    RFB.prototype = {
        ...
        _negotiate_aten_auth: function () {
            // var aten_sep = this._aten_password_sep;  // XXX: @KK: Fixme: should be wired up to the appropriate setting.
            var aten_sep = ':';
            var aten_auth = this._rfb_password.split(aten_sep);
            if (aten_auth.length < 2) {
                this._onPasswordRequired(this, 'ATEN iKVM credentials required (user' + aten_sep + 'password)');
                return false;
            }
        ...
        _negotiate_server_init: function () {
            ...
            if (this._rfb_atenikvm) {
                ...
                // XXX: @KK: Doing this will break interaction with non-ATEN servers until the page is reloaded; it also breaks the mouse/keyboard portions of the test suite!
