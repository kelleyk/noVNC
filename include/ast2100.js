...
// @KK: This is not properly part of the AST2100 decoder, but it's very related.  This function is just a little helper
...
(function () {
    ...
    Ast2100Decoder = function (defaults) {
        ...
        // Either 444u or 422u (though the "422" mode is really 4:2:0, where chroma is subsampled by a factor of two in
        // each direction)..  Applies only to JPEG-ish blocks, not to VQ blocks; but VQ blocks seem to only appear in
        // 4:4:4 color mode.
        ...
        // Allocate memory here once and then re-use it.
        // These buffers store data after entropy decoding (Huffman, zigzag) and before we dequant and apply the IDCT.
        this._tmpBufY = [
        ...
        // The final RGB output goes here; noVNC expects each group of four elements to represent one RGBA pixel.
    ...
    Ast2100Decoder.prototype = {
        ...
        _initializeVq: function () {
            // These colors are in YCbCr, so they're just black, white, and two shades of grey.
        ...
        decode: function (data) {
            ...
            if (traceUpdates)
                console.log("decode(): finished after "+mcuIdx+" blocks");  // one of which will have been the 0x9 "end-of-frame" block
        ...
        // - Always 8x8, since chroma subsampling does not apply to VQ-compressed data.  (As a consequence, VQ blocks only seem to
        //   appear when DCT chroma subsampling is disabled; that is, in "444" mode.)
        ...
        _parseVqBlock: function (codewordSize) {
            ...
            // Read new codebook data.  (That is: new colors for each of our codewords (the values we'll read from the
            // input data) to map to.)
            for (i = 0; i < (1 << codewordSize); ++i) {
                // Read 1b flag and 2b codebook slot number.  if flag is set, read 24b RGB value and set colors[slot #].
                // Regardless (?), set the ith codeowrd to map to this slot.
                ...
                if (hasNewColor) {
                    ...
                    // console.log(' - loaded VQ codebook color; codeword='+i+' slot='+codebookSlotIdx+' color='+fmt_u8a(color));
        ...
        _parseDataUnit: function (componentIdx, buf) {
            ...
            // First element is the DC component, followed by 63 AC components.  The DC component is encoded slightly
            // differently than the AC components: it is stored as the delta between the value and the last DC component
            ...
            for (var i = 1; i < 64;) {
                ...
                // @KK: Renamed r, s to runlen, size.
                // See ITU T.81 p89 (e.g. Fig F.1).
                // r is runlength of zeroes; if s==0, r==0 means EOB and r==15 means ZRL.
                // s is number of bits required to represent the amplitude that follows the codeword.
                var runlen = x >>> 4;
                var size = x & 0x0F;
        ...
        _readEncodedValueAC: function (category) {
            ...
            if (val_sign == 0) { // Negative (unlike a normal "signed int" representation).
                value = -(1 << category) + 1;
            } else {
                value = (1 << category - 1);
            }
