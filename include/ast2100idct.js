...
var range_limit = function (x) {
    x += 128;  // XXX: This is baked into the range limit table in the ATEN stuff and in libjpeg; see commpent above prepare_range_limit_table() in jdmaster.c.
...
var fixed_dequant = function (scaled_quant_table, buf, i) {
    // N.B.: The data in buf is unscaled; the data in scaled_quant_table has been scaled by 1<<16.
...
// @KK: This isn't a libjpeg parameter; it just means that ATEN downshifts all the way back to "normal ints" at the end of pass 1.
    ...
    AST2100IDCT = {
        ...
        _aan_idct_row: function(scaled_quant_table, buf, workspace, y) {
            ...
            var set_out = function (x, val) {
                // Shift right by PASS1_BITS bits to convert back to a normal int, and then by another 3 to divide by 8.
